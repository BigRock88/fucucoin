...
class ZerocoinSpendTest(FucucoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zfucu_bal, fucu_bal):
            zfucu_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zfucu_bal)
            fucu_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], fucu_bal)
            return zfucu_bal, fucu_bal
        ...
        zfucu_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zfucu_balance, 6666)
        ...
        zfucu_balance, balance = check_balances(denom_2, zfucu_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfucu_balance, balance = check_balances(denom_3, zfucu_balance, balance)
        ...
        _, _ = check_balances(denom_1, zfucu_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
