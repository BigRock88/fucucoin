...
{
    CDataStream serializedCoinSpend = ZFUCUModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZFUCUSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to zFUCU supply (mints are forever disabled after last checkpoint)
    ...
    //Remove spends from zFUCU supply
    ...
    // A one-time event where only the zFUCU supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZFUCUSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to zFUCU supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from zFUCU supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
