...
CzFUCUWallet::CzFUCUWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zFUCU seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFUCUSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFUCUSeed_deprecated()) {
                    LogPrintf("%s: Updated zFUCU seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zFUCU seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zFUCU wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFUCUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFUCUCount(nCountLastUsed);
    else if (!walletdb.ReadZFUCUCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFUCUWallet::Lock()
{
    seedMaster.SetNull();
}

void CzFUCUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFUCUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFUCU(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFUCUWallet::LoadMintPoolFromDB()
{
...
void CzFUCUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFUCUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFUCUWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zfucuTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFUCUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFUCU(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zFUCUTracker which also adds to database
    wallet->zfucuTracker->Add(dMint, true);
...
void CzFUCUWallet::SeedToZFUCU(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzFUCUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFUCUWallet::GenerateDeterministicZFUCU(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFUCUWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFUCUWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFUCUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
