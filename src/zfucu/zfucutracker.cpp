...
#include "zfucu/zfucuwallet.h"


CzFUCUTracker::CzFUCUTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzFUCUTracker::~CzFUCUTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFUCUTracker::Init()
{
...
bool CzFUCUTracker::Archive(CMintMeta& meta)
{
...
bool CzFUCUTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFUCUTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFUCUTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFUCUTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzFUCUTracker::GetSerialHashes()
{
...
CAmount CzFUCUTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFUCUTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFUCUTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFUCUTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFUCUTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFUCUTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFUCUTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFUCUTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFUCUTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFUCUTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzFUCUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFUCUWallet* zFUCUWallet)
{
    bool iszFUCUWalletInitialized = (nullptr != zFUCUWallet);
    CMintMeta meta;
    ...
    if (!iszFUCUWalletInitialized)
        zFUCUWallet = new CzFUCUWallet(wallet);
    meta.isSeedCorrect = zFUCUWallet->CheckSeed(dMint);
    if (!iszFUCUWalletInitialized)
        delete zFUCUWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFUCUTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFUCUTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFUCUTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFUCUTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFUCUTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFUCUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzfucu from DB\n", __func__, listDeterministicDB.size());
    }
