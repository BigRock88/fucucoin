...
bool CLegacyZFucuStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZFUCUStake from non zc-spend", __func__);
    ...
    *this = CLegacyZFucuStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zFUCU stake origin", __func__);

    // All good
    return true;
}

CLegacyZFucuStake::CLegacyZFucuStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZFucuStake::GetIndexFrom()
{
...
CAmount CLegacyZFucuStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZFucuStake::GetUniqueness() const
{
...
bool CLegacyZFucuStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zFUCU stake block: height %d outside range", __func__, nHeight);
