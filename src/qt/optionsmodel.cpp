...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::FUCU);
    ...
    if (settings.contains("nAnonymizeFucucoinAmount") || reset)
        SoftSetArg("-anonymizefucucoinamount", settings.value("nAnonymizeFucucoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fucucoin.cpp during shotdown to also remove the window positions
