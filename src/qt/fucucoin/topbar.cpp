...
#include "qt/fucucoin/forms/ui_topbar.h"
#include "qt/fucucoin/loadingdialog.h"
#include "qt/fucucoin/lockunlock.h"
#include "qt/fucucoin/qtutils.h"
#include "qt/fucucoin/receivedialog.h"
                                                        ...
                                                        ui(new Ui::TopBar)
{
    ...
    ui->pushButtonConf->setButtonText("fucucoin.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open fucucoin.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("FUCU")
    updateDisplayUnit();
