...
- [Development guidelines](#development-guidelines)
    - [General FUCU Core](#general-fucucoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
FUCU Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
FUCU Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/fucucoin910/fucucoin/blob/master/contrib/valgrind.supp))
...
However, in open source software such as FUCU Core, where everyone uses
...
pay attention to for reviewers of FUCU Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: FUCU Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against FUCU Core so that they can be integrated
...
file descriptors it will cause problems with FUCU's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:FUCU-Project/FUCU.git
